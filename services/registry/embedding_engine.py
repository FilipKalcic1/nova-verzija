...
TRANSLATION API INTEGRATION (v3.5):
    - DeepL/Google Translate API fallback for unmapped terms
    - Cached translations (30 days) to avoid repeated API calls
    - Use build_embedding_text_with_api() for API-enhanced embeddings
...
class EmbeddingEngine:
    ...
    async def build_embedding_text_with_api(
        self,
        operation_id: str,
        service_name: str,
        path: str,
        ...
        """
        Build embedding text with translation API fallback for unmapped terms.

        v3.5: Uses DeepL/Google Translate when dictionary lookup fails.
        This produces higher quality Croatian embeddings for unmapped terms.

        Usage:
            # Only use when you want API translations (slower but better)
            text = await engine.build_embedding_text_with_api(...)

        Returns:
            Embedding text with API-translated terms where available
        """
        # First, generate with standard method
        purpose = self._generate_purpose(method, parameters, output_keys, path, operation_id)

        # Collect unmapped terms from path and operation_id
        unmapped_terms = []

        # Check path segments
        path_segments = self._extract_path_segments(path)
        for segment in path_segments:
            if segment.lower() not in self.PATH_ENTITY_MAP:
                if len(segment) >= 4 and segment not in self.SKIP_SEGMENTS:
                    unmapped_terms.append(segment)

        # Check operation_id words
        words = self._parse_camel_case(operation_id)
        for word in words:
            if word.lower() not in self.PATH_ENTITY_MAP:
                if word.lower() not in self.OUTPUT_KEY_MAP:
                    if len(word) >= 4 and word.lower() not in self.ACTION_WORDS:
                        unmapped_terms.append(word)

        # Translate unmapped terms via API
        api_translations = {}
        if unmapped_terms:
            try:
                from services.registry.translation_helper import TranslationHelper
                helper = TranslationHelper(use_api=True)

                # Batch translate
                results = await helper.translate_batch_with_api(unmapped_terms)
                for term, result in zip(unmapped_terms, results):
                    if result.method == "api" and result.confidence >= 0.5:
                        api_translations[term.lower()] = result.translated
                        logger.debug(f"API translated for embedding: '{term}' -> '{result.translated}'")
            except Exception as e:
                logger.warning(f"API translation failed during embedding: {e}")

        # Rebuild purpose with API translations
        if api_translations:
            # Re-generate with API translations available
            purpose = self._generate_purpose_with_api(
                method, parameters, output_keys, path, operation_id, api_translations
            )

        # Build final embedding text (same as sync version)
        parts = [
            purpose,
            description if description else "",
        ]
    ...
    def _generate_purpose_with_api(
        self,
        method: str,
        parameters: Dict[str, ParameterDefinition],
        ...
        """
        Generate purpose using API translations for unmapped terms.

        Same logic as _generate_purpose but uses api_translations dict
        for terms not found in PATH_ENTITY_MAP.
        """
        action = self.METHOD_TO_CROATIAN.get(method.upper(), "Obrađuje")
        entities, action_hint = self._parse_operation_id_with_api(operation_id, api_translations)

        if action_hint:
            action = action_hint

        path_entities = self._extract_entities_from_path_with_api(path, api_translations)

        all_entities = list(dict.fromkeys(path_entities + entities))[:3]
        ...
        if output_keys:
            ...
            for key in output_keys[:3]:
                key_lower = key.lower()
                if key_lower in self.OUTPUT_KEY_MAP:
                    output_parts.append(self.OUTPUT_KEY_MAP[key_lower])
                elif key_lower in api_translations:
                    output_parts.append(api_translations[key_lower])
            if output_parts:
                output_desc = f" Vraća {', '.join(output_parts)}."

        if all_entities:
            entity_str = " i ".join(all_entities[:2]) if len(all_entities) >= 2 else all_entities[0]
            purpose = f"{action} {entity_str}.{output_desc}"
        else:
            purpose = f"{action} podatke.{output_desc}"

        return purpose

    def _extract_entities_from_path_with_api(
        self,
        path: str,
        api_translations: Dict[str, str]
    ) -> List[str]:
        """
        Extract entities from path with API translation fallback.
        """
        entities = []
        ...
        for segment in segments:
            ...
            if segment_lower in self.PATH_ENTITY_MAP:
                _, genitive = self.PATH_ENTITY_MAP[segment_lower]
                entities.append(genitive)
            elif segment_lower in api_translations:
                entities.append(api_translations[segment_lower])
            elif len(segment) >= 4:
    ...
    def _parse_operation_id_with_api(
        self,
        operation_id: str,
        api_translations: Dict[str, str]
    ) -> tuple:
        """
        Parse operation ID with API translation fallback.
        """
        if not operation_id:
            return [], None
        ...
        for word in words:
            ...
            if word_lower in self.ACTION_WORDS:
                continue
            if len(word) < 3:
                continue

            if word_lower in self.OUTPUT_KEY_MAP:
                action_hint = self.OUTPUT_KEY_MAP[word_lower]
            elif word_lower in self.PATH_ENTITY_MAP:
                _, genitive = self.PATH_ENTITY_MAP[word_lower]
                entities.append(genitive)
            elif word_lower in api_translations:
                entities.append(api_translations[word_lower])
            elif len(word) >= 4:
                readable = self._make_readable(word)
                entities.append(readable)

        return entities[:3], action_hint
