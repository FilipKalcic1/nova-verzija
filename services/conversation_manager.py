...
class ConversationManager:
    ...
    def parse_confirmation(self, user_input: str) -> Optional[bool]:
        ...
        # Handle ambiguous phrases first - these should return None
        ambiguous_phrases = ["ne znam", "možda", "nisam siguran", "not sure"]
        ...
        # Split into words for exact matching (avoid "može" matching "možda")
        words = set(cleaned.split())

        confirm_words = {"da", "potvrdi", "ok", "potvrđujem", "može", "yes", "uredu", "y"}
        cancel_words = {"ne", "odustani", "cancel", "odustajem", "prekini", "stop", "no", "n"}

        # Check for exact word matches
        if words & confirm_words:  # Set intersection
            return True
