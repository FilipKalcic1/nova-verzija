...
class DataSanitizer:
    ...
    def _sanitize_dict(self, data: Dict, depth: int) -> Dict:
        ...
        for key, value in data.items():
            ...
            # Completely redact sensitive keys (exact match only)
            # This allows "credentials" dict to be recursively processed
            # while "password" field inside gets redacted
            if key_lower in self.SENSITIVE_KEYS:
                result[key] = "[REDACTED]"

            # Partially mask certain keys (exact match only)
            elif key_lower in self.PARTIAL_MASK_KEYS:
    ...
    def _sanitize_string(self, text: str) -> str:
        ...
        # CRITICAL: Process full-redaction patterns FIRST (credentials)
        # Otherwise partial-mask patterns (like phone) corrupt the string
        # before credential patterns can match
        credential_patterns = ("password", "api_key", "jwt", "bearer")

        # First pass: Full redaction for credentials
        for name in credential_patterns:
            if name in self.PATTERNS:
                result = self.PATTERNS[name].sub("[REDACTED]", result)

        # Second pass: Partial mask for other sensitive data
        for name, pattern in self.PATTERNS.items():
            if name not in credential_patterns:
                result = pattern.sub(
