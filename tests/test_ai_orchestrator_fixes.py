...
FINAL_TOKEN_OVERHEAD = 3


class MockAIOrchestrator:
    """
    Mock AIOrchestrator for testing without Azure OpenAI credentials.
    Implements the methods being tested.
    """

    def __init__(self):
        self.tokenizer = None  # No tiktoken for fallback testing
    ...
    def _apply_smart_history(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        """Apply smart history - returns messages as-is for testing."""
    ...
    def _apply_token_budgeting(
        ...
        """Apply token budgeting with alignment check."""
        if not tool_scores:
            return tools
    ...
    def _format_entity_context(self, entities: Dict[str, List[str]]) -> str:
        """Format entity context from extracted entities."""
        parts = []
...
def test_scenario_6_token_counting_accuracy():
    ...
    # MockAIOrchestrator always has tokenizer=None (tests fallback)
    orchestrator = AIOrchestrator()
    ...
    expected = int(len(messages[0]['content']) / 4.6) + MESSAGE_TOKEN_OVERHEAD
    assert token_count == expected, f"Expected {expected}, got {token_count}"
